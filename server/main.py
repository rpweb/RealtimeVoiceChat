    ...
    async def call_runpod_function(self, payload: dict) -> dict:
        ...
        try:
            ...
            if chunks_to_process:
                logger.info(f"📦 Processing {len(chunks_to_process)} chunks from RunPod")
                for chunk in chunks_to_process:
                    chunk_type = chunk.get("type", "")
                    logger.debug(f"📡 Sending chunk: {chunk_type}")
                ...
                logger.warning(f"⚠️ No chunks found in stream response")
